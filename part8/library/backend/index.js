...
const resolvers = {
  Query: {
    ...
    allAuthors: () => {
      const bookCountBy = _.countBy(books, book => book.author)
      return authors.map(author => ({...author, bookCount: bookCountBy[author.name]}))
    }
  },
  Mutation: {
    addBook: (root, args) => {
      ...
      // add author if not already existing
